...
from .transliterator import OttomanTransliterator, TransliterationMapping
...
def load_stopwords(path: str | Path) -> set[str]:
    ...
    try:
        ...
        raise ResourceError(f"Failed to load stopwords from {path}: {e}") from e
...
def load_suffixes(path: str | Path) -> list[str]:
    ...
    try:
        ...
        raise ResourceError(f"Failed to load suffixes from {path}: {e}") from e
...
class OttomanProcessor:
    ...
    def process(self, text: str) -> OttomanProcessingResult:
        ...
        # Always create a mapping for offset preservation, even when not
        # auto-transliterating
        if script_type == "arabic":
            ...
            if self.config.auto_transliterate:
                ...
                # For tokenization, still need to normalize, but will use
                # original tokens
                text_for_tokenization = full_mapping.transliterated
        ...
        tokens = tokenize(
            text_for_tokenization, strip_punct=self.config.remove_punctuation
        )
        
        # Step 6: Build results with proper transliteration mappings
        processed_tokens = []
        original_tokens_result: list[str] | None = (
            [] if self.config.preserve_original else None
        )
        offset_mappings = []
        script_types = []
        ...
        if full_mapping is not None:
            ...
            for token in tokens:
                ...
                if orig_end >= 0:
                    for _o_s, o_e, t_s, t_e in full_mapping.char_mappings:
                ...
                if orig_start >= 0 and orig_end >= orig_start:
                    ...
                    # Fallback: use token-local offsets if we could not
                    # resolve global ones
                    token_char_mappings = [
        ...
        if self.stopwords:
            ...
            if original_tokens_result is not None:
                original_tokens_result = [
                    original_tokens_result[i] for i in filtered_indices
                ]
    ...
    def get_bounding_box_info(
        ...
        return {
            ...
            "offsets": offsets,
            "script_type": (
                result.script_types[token_index]
                if token_index < len(result.script_types)
                else None
            ),
        }
